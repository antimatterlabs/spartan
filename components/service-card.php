 ...
 */
$service_url = $service['url'] ?? 'inspection-services.php';
?>
<div class="group">
    <a href="<?php echo site_escape($service_url); ?>" class="block h-52 w-full overflow-hidden mb-6">
    ...
    <h3 class="font-oswald text-lg font-bold text-spartan-navy tracking-widest uppercase mb-3">
        <a href="<?php echo site_escape($service_url); ?>" class="hover:text-spartan-teal transition-colors"><?php echo $service['title']; ?></a>
    </h3>
    <p class="text-sm text-slate-600 font-light leading-relaxed mb-5">
        <?php echo $service['desc']; ?>
    </p>
    <a href="<?php echo site_escape($service_url); ?>" class="text-spartan-teal hover:text-spartan-navy text-[10px] font-bold tracking-[0.2em] inline-flex items-center uppercase transition-colors">
